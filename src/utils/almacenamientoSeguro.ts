...
class AlmacenamientoSeguro {
  private readonly SERVICIO_KEYCHAIN = 'PuntoRideDriver';
  private readonly CLAVE_TOKEN_LEGACY = 'token';
  ...
  async eliminarCredenciales(): Promise<boolean> {
    try {
      const resultado = await Keychain.resetInternetCredentials(this.SERVICIO_KEYCHAIN);
      await AsyncStorage.removeItem(this.CLAVE_TOKEN_LEGACY);
      return resultado;
    } catch (error) {
